...
con cualquier algoritmo, ronda entre 3.4 y 4.1 hogares atendidos por cada
hogar que realmente lo necesitaba (Tabla~\ref{tab:matriz_confusion_a}). A
la vez, de los hogares que efectivamente no entran en pobreza, los modelos
descartan correctamente entre el 26\% (XGBoost) y el 47\% (LightGBM); el
resto recibe una alerta que no necesitaba. En
...
El comportamiento de recall y precision obedece
no solo al modelo, sino en parte también a la decisión de diseño: el
umbral elegido por F$_2$, que da el doble de peso a no dejar sin detectar
a hogares que caen en pobreza (falsos negativos) que a marcar de más a
hogares que no caen (falsos positivos), determina en qué punto del
intercambio entre \emph{recall} y precisión se ubica el modelo. Lo que sí
...
y estrategia de balanceo de clases elegidos por validación cruzada para

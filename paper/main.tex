...
Dos patrones organizan esta comparación, más allá de las diferencias
puntuales entre algoritmos y especificaciones (Tabla~\ref{tab:desempeno_modelos}).
\textbf{Primero,} el algoritmo elegido
no cambia el poder de discriminación (AUC-ROC 0.731--0.767 en los cinco),
pero sí determina cuánta capacidad de cobertura necesita un programa para
no perder casos reales --y ese costo, con cualquier algoritmo, ronda los
3--4 hogares atendidos por cada 1 que realmente lo necesitaba
(Tabla~\ref{tab:matriz_confusion_a}). La comparación entre algoritmos es
indicativa, no concluyente: sus intervalos de confianza se traslapan y no
se corrió una prueba pareada (DeLong) que la haga formal. El costo de
cobertura sí varía de forma notable entre ellos incluso con AUC casi
idéntico --HistGradientBoosting identifica correctamente al 40\% de los
hogares seguros, XGBoost solo al 26\%--, y en parte es una decisión de
diseño: el umbral elegido por F$_2$, no solo el algoritmo, determina dónde
se cae dentro del techo que impone el AUC. Lo que sí es una restricción
real, de la que ninguna combinación de algoritmo y umbral escapa, es que
con un AUC-ROC de 0.75 no es posible lograr recall y precision altos
simultáneamente. Bajo IPM el mismo techo aparece, en un rango de AUC
igual de estrecho (0.725--0.747; Tabla~\ref{tab:desempeno_modelos_ipm},
Anexo~\ref{apx:desempeno_ipm}).

\textbf{Segundo,} ese costo apenas cambia si se excluye el ingreso/gasto
del hogar de las covariables --la ganancia de incluir esa información, la
más cara de levantar, es marginal (brecha de AUC de 0.008 a 0.028 entre
...
implica que el ingreso no importe --univariadamente es de las variables
...
ya está sustancialmente duplicado por otras variables del núcleo del
perfil (riqueza, educación, bienes durables; Hallazgo 4,
Sección~\ref{subsec:caracterizacion_grupos}). Bajo IPM la brecha es aún
menor, y para Random Forest y XGBoost es negativa --el Modelo B supera al
A--, lo que descarta que la brecha chica bajo monetaria se deba a la
relación mecánica entre ingreso y la definición de esa pobreza: ni
siquiera donde el ingreso no define el resultado ayuda incluirlo. La

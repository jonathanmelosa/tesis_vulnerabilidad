...
Para evaluar el riesgo de multicolinealidad se calculó la correlación de
Pearson entre las 159 variables numéricas del consolidado —incluidas las
4 geoespaciales—. Para evitar correlaciones espurias por pocas
observaciones conjuntas no-nulas (un riesgo real dado que varios módulos
tienen cobertura parcial, Tabla~\ref{tab:cobertura_modulos}), solo se
reportan como ``altas'' los pares con $|r|>0.7$ calculados sobre al menos
500 observaciones conjuntas; con ese filtro se identifican los 35 pares
de la Figura~\ref{fig:top_correlaciones}.
...
matrices de transición de pobreza monetaria e IPM cruzando el estado de pobreza
...
hogares que se encuentran por encima de la LP en el periodo inicial $t$, es decir, 
los que inicialmente no eran pobres y que posteriormente caen en pobreza. 
...
\noindent El objetivo entonces es estimar la probabilidad condicional de caer en
...
estado inicial y final únicos. Las matrices de transición resultantes, y
su interpretación se presentan en la
...
patrón que generaliza, sin gastar el conjunto de prueba, que solo se toca
una vez, al final. Se eligen los 30 hiperparámetros por costo
...
estrategias de balanceo (Sección~\ref{subsec:desbalance}) no era viable en
tiempo razonable--; la métrica que decide qué combinación gana esa
búsqueda es AUC-ROC, por la misma razón que se usa como métrica principal
de desempeño (párrafo siguiente).
...
literatura, aunque no es la métrica que decide dónde queda el umbral. El
foco del análisis está en comparar el desempeño relativo
...
falso negativo (un hogar que sí cae en pobreza y el modelo no marca como
riesgo) y un falso positivo (un hogar que el modelo marca como riesgo y no
...
pobreza tiene un costo --no recibe el apoyo que necesitaría-- mayor que
incluir de más a un hogar que no cae, que en el peor caso implica un gasto
de focalización mal dirigido pero no la ausencia de una protección que sí
hacía falta. F$_2$ formaliza esa prioridad ponderando el recall el doble
...
\subsection{Dinámica de transiciones a la pobreza en la ELCA}
\label{subsec:transiciones}

Esta sección caracteriza las transiciones entre estados de pobreza en el
panel ELCA 2010--2013--2016, bajo las dos definiciones de esta tesis
--monetaria por ingreso (LP del DANE) e IPM (Sección~\ref{subsec:outcome})--,
según las cuatro categorías de \textcite{lopezCalva2014} (nunca/siempre
pobre, sale/entra en pobreza). Incidencia, FGT y cinco ejercicios de
robustez frente a decisiones de construcción del ingreso y
representatividad muestral --incluida la atrición del panel por perfil de
hogar-- se documentan en el Apéndice~\ref{apx:transiciones_robustez}.

Las matrices de transición (Tabla~\ref{tab:matriz_transicion}) cruzan el
estado inicial (filas) contra el final (columnas), en \% de fila, para
hogares con emparejamiento 1 a 1 entre olas (excluye 511 hogares
divididos en 2010--2013 y 1{,}190 en 2013--2016).
...
salir (45.5\% y 47.7\%, frente a 29.7\%--38.2\% en monetaria)
--consistente con que el IPM mide privaciones estructurales (vivienda,
educación, servicios) que cambian más lento que el ingreso corriente en
cualquiera de las dos direcciones.

Las Figuras~\ref{fig:sankey_2010_2013} y~\ref{fig:sankey_2013_2016}
traducen la matriz monetaria a flujos entre las cuatro categorías de
\textcite{lopezCalva2014}: en 2010--2013 dominan los \emph{siempre
pobres} (43.9\%) sobre los \emph{nunca pobres} (28.8\%); en 2013--2016 el
orden se invierte (37.0\% vs.\ 33.3\%). En ambos periodos la salida
(18.6\%, 20.6\%) casi duplica la entrada (8.8\%, 9.2\%): la pobreza
monetaria se redujo de forma neta, no solo se redistribuyó entre hogares.

\begin{figure}[H]
  \centering
  \begin{minipage}{0.49\textwidth}
    \centering
    \includegraphics[width=0.82\textwidth]{../outputs/figures/pobreza/sankey_transicion_pobreza_2010_2013.png}
    \caption{2010$\to$2013 ($n=8{,}218$).}
    \label{fig:sankey_2010_2013}
  \end{minipage}\hfill
  \begin{minipage}{0.49\textwidth}
    \centering
    \includegraphics[width=0.82\textwidth]{../outputs/figures/pobreza/sankey_transicion_pobreza_2013_2016.png}
...
Un ejercicio adicional sigue a los mismos hogares (no pobres en la ola
base) a lo largo de las tres olas para distinguir entradas transitorias de
persistentes (Tabla~\ref{tab:trayectorias_3olas}). Bajo pobreza monetaria,
24.1\% de los no pobres en 2010 entra en pobreza en algún momento; algo
más de la mitad sale de nuevo antes de 2016 (14.0 puntos, ``transitorio'')
y el resto se queda (10.1 puntos, ``persistente''). Bajo IPM la entrada
total es menor (12.2\%) y se reparte parecido (8.2 transitoria, 4.0
persistente). En ambas definiciones, más de la mitad de quienes entran en
pobreza no quedan atrapados dentro de la ventana de seis años observada.
...
estado y ventana acumulada (Sección~\ref{subsubsec:estado_acumulado}) --
...
0.731 y 0.767, con intervalos de confianza sobre 5 semillas del ajuste
...
regresión logística regularizada, pero en el Modelo B ese orden se rompe a
la mitad --Random Forest y XGBoost se mantienen por encima de la
logística, mientras que HistGradientBoosting y LightGBM caen por debajo de
ella--. Tercero, y más importante para el
...
sorprendentemente pequeña (0.008--0.028 en AUC-ROC según el algoritmo):
...
política: XGBoost maximiza el recall --relevante si el costo
...
inclusión)--, mientras que HistGradientBoosting ofrece mejor precision y el
mejor balance entre ambas (mayor F1) al mismo nivel de AUC-ROC. Un sistema real debería fijar explícitamente esa

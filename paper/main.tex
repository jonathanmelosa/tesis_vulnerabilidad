...
Queda pendiente, como extensión de este análisis
(Sección~\ref{subsec:limitaciones}), caracterizar específicamente a los
hogares que constituyen los errores de inclusión (falsos positivos) --es
decir, comparar su perfil contra el de los hogares correctamente
clasificados (verdaderos positivos y verdaderos negativos) usando el
núcleo de variables ya validado en la Sección~\ref{subsec:determinantes},
para entender no solo cuántos son sino a quiénes se está incluyendo por
error--.
...
\subsubsection{Importancia de variables e interpretabilidad}
...
\input{tables/tab_shap_nucleo_perfil.tex}
  \begin{minipage}{0.95\textwidth}
    \vspace{4pt}
...
fuerza, entre las posiciones 4 y 20 de forma consistente en los cinco
algoritmos. Esto no contradice ni invalida el Hallazgo 2: la importancia
...
sí solo no captura. Para el diseño de política esto tiene dos
implicaciones: el proxy operacional recomendable para un sistema de
alerta temprana es la tasa de cotización a pensión del hogar y no la
condición de asalariado del jefe --y es, además, el tipo de dato que ya
reside en registros administrativos de seguridad social, en el mismo
espíritu de cruce de fuentes que plantea el CONPES 3877/SISBEN IV
\parencite{conpes2016}--, pero, en un sentido más general, un hallazgo
...
(sin evidencia de eso tampoco, en los tres terciles de cercanía). Ver
\texttt{src/05\_model/modelo\_ablacion\_sin\_riqueza.py} y
\texttt{src/05\_model/heterogeneidad\_cerca\_umbral.py}.} La razón es
...
pero esa correlación colapsa a $r\approx+0.021$ al controlar por dos
preguntas que la ELCA ya hace (número de servicios públicos y de bienes
durables del hogar) --las mismas dos variables que dominan el núcleo de
importancia multivariada (Sección~\ref{subsec:importancia_resultados})--.
...
ejes explorados (Tabla~\ref{tab:heterogeneidad_dmsp}, Anexo~\ref{apx:heterogeneidad_dmsp}):
...
subpoblación (\texttt{diagnostico\_heterogeneidad\_ipm.csv}).

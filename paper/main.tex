...
Esta sección presenta los resultados en cinco partes. La
Sección~\ref{subsec:caracterizacion_grupos} perfila al hogar vulnerable
--el que entra en pobreza-- contra los otros tres grupos de la matriz de
transición, para las 53 (monetaria) y 36 (IPM) covariables más robustas a
esa tipología. La Sección~\ref{subsec:desempeno} compara el desempeño
fuera de muestra de los cinco algoritmos bajo las especificaciones con y
sin ingreso/gasto del hogar. Las Secciones~\ref{subsec:marginal}
y~\ref{subsec:marginal_ipm} evalúan si agregar DMSP-OLS mejora esa
predicción, bajo pobreza monetaria e IPM respectivamente, y por qué. La
Sección~\ref{subsec:heterogeneidad} cierra preguntando si ese efecto
--nulo en promedio bajo monetaria, pequeño bajo IPM-- está parejo en la
...
(Sección~\ref{subsec:transiciones}), sobre las 53 covariables
individualmente robustas a esta tipología (efecto --$\eta^2$/V de Cramér
sesgo-corregida, ponderados-- superior a 0.10,\footnote{40 de 53; las 13
restantes cumplen efecto$>$0.05 y robustez en \emph{ambas} transiciones por
separado. Las 40 originales no se sometieron a ese chequeo: 2
(\texttt{n\_ninos\_12}, \texttt{razon\_dependencia\_demografica}) caen bajo
0.10 en 2013$\to$2016 (0.105$\to$0.084 y 0.104$\to$0.088), aunque siguen
robustas.} con bootstrap de 300 réplicas), sin colapsar las
correlacionadas entre sí, en una sola tabla consolidada
(Tabla~\ref{tab:perfil_consolidada}).

Tres hallazgos organizan esta sección. \textbf{Primero,} el hogar
...
Tabla~\ref{tab:perfil_consolidada}. Esto es consistente con pensar
...
\textbf{Segundo,} y más importante para el argumento de esta tesis: cuatro
variables (marcadas $^\dagger$ en la tabla) rompen ese patrón, todas en la
misma dirección --el hogar vulnerable queda en el extremo \emph{más
favorable}, no en el segundo peor--. La más importante es la formalidad
...
quien sale (52.2\%) y más que quien nunca cae (57.9\%). Las otras tres son
menores pero van en la misma dirección: deuda informal (26.0\% frente a
22.7\%), niños al cuidado de terceros (98.9\% frente a 98.2\%) y
transporte público comunitario (79.2\% frente a 78.2\%). Si una política
...
en pobreza'' no es homogéneo. De los 723 hogares que entran entre 2010 y
2013, 610 tienen dato de 2016: 355 (58\%) vuelven a salir (``transitorio'')
y 255 (42\%) se quedan pobres (``persistente'',
Tabla~\ref{tab:perfil_split_trayectoria}). El patrón ``se parece a sale''
sobrevive en ambos subgrupos (39/53 y 44/53 variables más cerca de
``sale'' que de ``nunca cae''), pero hay un gradiente hacia el extremo
crónico que el agregado diluye: persistente queda más cerca de ``siempre
pobre'' que de ``sale'' en 11/53 variables, transitorio en solo 2/53. La
divergencia más marcada es la deuda informal: el agregado (26.0\%)
esconde que transitorio (17.3\%) queda por \emph{debajo} de quien sale
(22.7\%), mientras persistente (41.9\%) supera a todos los demás grupos,
incluido el siempre pobre (31.6\%) --separa, dentro del propio grupo
...
\paragraph{Geografía.} La luz nocturna del hogar vulnerable (36.4 en
media, DMSP-OLS 0--63) separa monotónicamente los cuatro grupos, en media
y mediana, para monetaria e IPM (Figura~\ref{fig:dmsp_monotonico}) --con
una salvedad: en mediana, ``más cerca de sale'' es mucho más débil bajo
monetaria (Entra a 20.7 de Sale y 20.9 de Nunca, casi equidistante) aunque
se mantiene fuerte bajo IPM (2.9 de Sale, 38.4 de Nunca). Además, la
Sección~\ref{subsec:marginal} muestra que esta separación es en gran parte
redundante con servicios públicos y bienes durables ya en la encuesta: DMSP
separa bien, pero aporta poca información nueva.\footnote{ALOS PALSAR
(radar) sigue el mismo gradiente; Landsat NDVI lo invierte (más verde en
pobreza persistente, por ruralidad) --exploratorias, sin el mismo filtro
de robustez.}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.62\textwidth]{../outputs/figures/eda_transicion_covariables/dmsp_monotonico_categoria.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{../outputs/figures/mapa_territorial_divipola/prueba_estilo_tramas_regional.png}
...
\paragraph{Pobreza multidimensional (IPM).} El mismo ejercicio para IPM
(36 variables robustas, Anexo~\ref{apx:tablas}) muestra un patrón
cualitativamente distinto: quien entra en pobreza sí muestra privación
visible \emph{antes} de la transición --20\% vive en piso de tierra
(frente a 5\% de los nunca pobres), 34\% tiene acueducto público (frente a
64\%), 67.2\% vive en zona rural (frente a 36.5\%)--, más parecido al
siempre pobre --el espejo del patrón monetario: donde el ingreso ve un
choque súbito sobre un hogar que se parecía al seguro, la privación
multidimensional ve la continuación de un deterioro que ya se parecía al
persistente (Sección~\ref{subsec:transiciones})--. De las 53 variables
monetarias y las 36 de IPM, 28 son comunes (vivienda, ingreso/gasto,
educación y ocupación del jefe, riqueza, bienes durables, servicios
públicos, DMSP): el núcleo estructural no depende de cómo se mida la
pobreza. Las 25 exclusivas de monetaria son mayoritariamente financieras y
comunitarias; las 8 de IPM incluyen, de forma reveladora, la pobreza
monetaria del propio hogar en 2010 (\texttt{pobre\_ingreso} y
análogas) --es, en sí misma, uno de los mejores predictores de caer en
pobreza multidimensional, sin que lo inverso se sostenga igual.

\paragraph{Síntesis.} El hogar vulnerable ocupa, en la mayoría de sus
rasgos, la misma posición relativa que uno que apenas cruzó la línea hacia
el lado seguro --nada lo señala de antemano--. Por eso importa tanto la
excepción del empleo formal: en la única dimensión donde más se separa, lo
hace en la dirección \emph{opuesta} a la que sugeriría una foto fija; y la
deuda informal muestra que ni siquiera dentro del propio grupo vulnerable
esa foto distingue a quien va a recuperarse de quien va a quedar atrapado.
La vulnerabilidad no es un perfil identificable por carencias en un punto
del tiempo, sino una posición relativa a un umbral que ningún indicador de
estado revela --y que el más usado para descartarlo, activamente esconde.

\begin{table}[H]
  \centering
  \caption{$\Delta$AUC-ROC de agregar DMSP-OLS por subgrupo, pobreza multidimensional (IPM). En \textbf{negrita}: $|\Delta|\geq 0.02$. Grupos con $n<30$ omitidos (no confiables). XGB=XGBoost, RF=Random Forest, LGBM=LightGBM, HGB=HistGradientBoosting, Log.=Logística regularizada. Fuente: cálculos propios, \texttt{diagnostico\_heterogeneidad\_ipm.csv} (\texttt{src/05\_model/diagnostico\_heterogeneidad\_ipm.py}).}

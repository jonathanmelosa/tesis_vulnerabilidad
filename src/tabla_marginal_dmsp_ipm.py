"""
tabla_marginal_dmsp_ipm.py
=====================================
Version de `tabla_marginal_dmsp_fbeta2_cv10.py` para el target IPM
(pobreza multidimensional) en vez de pobreza monetaria -- lee
`registro_modelos_ipm.csv` (generado por
`src/05_model/modelo_ipm_comparacion.py`, misma config robusta:
F-beta=2, CV_FOLDS=10, N_ITER_BUSQUEDA=30). Mismo formato exacto que la
tabla monetaria, para comparacion directa lado a lado.
...
OUTPUTS
    ...
    paper/tables/tab_marginal_dmsp_ipm.tex

CÓMO CORRER

    python src/tabla_marginal_dmsp_ipm.py
"""
...
CONFIG = {
    "registro_modelos": REPO_ROOT / "data" / "processed" / "benchmark_resultados" / "registro_modelos_ipm.csv",
    ...
    "etiqueta_especificacion": {"Aipm": "A (IPM)", "AipmgeoDMSP": "A (IPM) + DMSP-OLS", "Bipm": "B (IPM)", "BipmgeoDMSP": "B (IPM) + DMSP-OLS"},
...
def cargar_registro(cfg: dict) -> pd.DataFrame:
    ...
    if not ruta.exists():
        print(f"ERROR: no se encontró {ruta} -- correr primero src/05_model/modelo_ipm_comparacion.py", file=sys.stderr)
        sys.exit(1)
...
def fila_tex(df: pd.DataFrame, algoritmo_raw: str, espec: str, cfg: dict) -> str:
    ...
    return (
        f"    {nombre:<24s} & {etiqueta:<22s} & {fila['auc_roc_media']:.3f} & "
...
def generar_tex(df: pd.DataFrame, cfg: dict) -> str:
    lineas = [
        ...
        r"  DMSP-OLS, target IPM (pobreza multidimensional), holdout temporal",
        r"  (train 2010$\to$2013, test 2013$\to$2016). Umbral elegido por CV",
        r"  maximizando F-beta ($\beta=2$), con CV\_FOLDS=10 y",
        r"  N\_ITER\_BUSQUEDA=30 -- misma configuración que",
        r"  Tabla~\ref{tab:marginal_dmsp_fbeta2_cv10} (pobreza monetaria).}",
        r"  \label{tab:marginal_dmsp_ipm}",
        r"  \footnotesize",
...
def main() -> None:
    ...
    ruta_tex = out_dir / "tab_marginal_dmsp_ipm.tex"
